...
| 翻译链路性能改动（首字/首句延迟等） | `npm run test:watch-mode-evidence:strict` | strict 模式对 `firstVisibleTranslationLatencySeconds` 等已产出延迟字段做阈值断言（默认 8s/15s，可用 `--latency-thresholds 字段=秒` 覆盖或 `=off` 关闭）；发布证据必须由 `npm run test:watch-mode-live:production-coordinator` 的单机串行入口产出，旧 `test:watch-mode-live:matrix` strict 路径会在任何 Provider 调用前拒绝；脚本自测用 `npm run test:watch-mode-report` |
