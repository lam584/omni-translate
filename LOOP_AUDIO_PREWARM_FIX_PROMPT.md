...
你正在维护 Windows 桌面应用 `E:\omni-translate`。请以资深 Windows 音频架构师、Rust/Tauri 工程师和测试负责人身份，自主执行“调查 → 建模 → 修改 → 验证 → 复盘”的闭环，持续推进，直到下面定义的完成条件全部满足。不要只给建议或停在分析报告；你需要实际检查代码、收集证据、实施修复并运行验证。
