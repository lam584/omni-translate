...
fn validate_error_object(event: &Value, require_type: bool) -> Result<(), String> {
    let error = event
        .get("error")
    ...
    {
        return Err("model_protocol.payload_invalid: optional error.param must be string or null".to_string());
    }
    Ok(())
}

fn response_id(event: &Value) -> Result<&str, String> {
...
fn snapshot_identity(event_type: &str, event: &Value) -> Result<String, String> {
...
fn snapshot_text(event: &Value) -> Result<String, String> {
