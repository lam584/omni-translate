...
fn replay_gate_timeout_then_late_completed() {
    ...
    drop(socket);
    let late_socket = harness.tick(late_socket, &mut slice);
    ...
    assert!(
        ...
        "a display-only late final must release the mutable input cue"
    );

    let late_cue_id = snapshot
        .subtitle_overlay
        .recent_cues
        .iter()
        .find(|cue| cue.source_text.contains("the user's last sentence"))
    ...
    let current_socket = ScriptedRealtimeSocket::new(
        vec![ScriptStep::Event(json!({
            "type": "conversation.item.input_audio_transcription.completed",
            "item_id": "item-current",
            "transcript": "the current turn"
        }))],
        harness.shared.clone(),
    );
    drop(late_socket);
    ...
    let late_cue = snapshot
        ...
        .recent_cues
        .iter()
        .find(|cue| cue.cue_id == late_cue_id)
