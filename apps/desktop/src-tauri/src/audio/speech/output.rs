    ...
    F: FnMut(u32, &str),
{
    ...
    loop {
        ...
        let readiness = (|| {
            ...
            let resolved_id = device.get_id()
                .map_err(|error| speaker_render_stage_error("endpoint-id", error))?;
            if let Some(requested_id) = device_id.filter(|id| {
                !matches!(
                    id.trim(),
                ...
                if resolved_id != requested_id {
                    return Err(format!("speaker-render-stage=endpoint-identity error=resolved endpoint mismatch requested={requested_id} resolved={resolved_id}"));
                }
            }
            ...
            let mut audio_client = device
                .get_iaudioclient()
                .map_err(|error| speaker_render_stage_error("readiness-audio-client", error))?;
            ...
            if buffer_frames == 0 {
                return Err(
                    "speaker-render-stage=readiness-buffer-size error=zero-frame buffer"
                        .to_string(),
                );
            }
            Ok(resolved_id)
