...
async fn debug_ipc_ping<R: tauri::Runtime>(
    ...
    let watch_diagnostic = watch_mode_diagnostic::autostart_enabled();
    log_debug!(
        &app,
        "runtime",
        "debug_ipc_ping",
        format!(
            "status={} elapsedMs={} watchDiagnostic={} backendAutostartAuthoritative={}",
            snapshot.status,
    ...
    Ok(watch_mode_diagnostic::build_debug_ipc_ping_response(
        &snapshot.status,
        start.elapsed().as_millis(),
        watch_diagnostic,
    ))
...
fn main() {
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            let render_receipt_app = app_handle.clone();
            app.listen("subtitle-overlay://rendered", move |event| {
                if let Ok(receipt) = serde_json::from_str::<
                    audio::contracts::OverlayRenderReceiptRuntime,
                >(event.payload())
                {
                    render_receipt_app
                        .state::<AudioStateStore>()
                        .watch_session_report
                        .record_overlay_receipt(receipt);
                }
            });
            ...
            if watch_mode_diagnostic::should_run_idle_overlay_prewarm(
                watch_mode_diagnostic::autostart_enabled(),
            ) {
                ...
                let _ = append_diagnostics_log(
                    ...
                    "runtime",
                    "info",
                    "watch_mode.diagnostic_overlay_idle_prewarm_skipped",
                    Some("reason=diagnostic-route-owns-overlay-window".to_string()),
                    None,
                    None,
                );
            }

            // Diagnostic autostart is opt-in. Schedule it from setup, but do
