...
mod artifacts;
#[path = "release_evidence_diagnostic/provider_probe.rs"]
mod provider_probe;
...
use provider_probe::{collect_provider_config, collect_provider_probe};
...
async fn run(app: &AppHandle) -> Result<(), String> {
    ...
    let mut artifact_paths = scenario.artifact_paths().to_vec();
    if scenario == EvidenceScenario::ProviderProbe && staging.join("raw").is_dir() {
        artifact_paths.push("raw");
    }
