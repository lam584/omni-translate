...
export default function GlossaryEntryDialog(props: Props) {
  ...
  return <ModalDialog aria-label={props.state.id ? t('glossary.dialog.editEntryTitle') : t('glossary.dialog.addEntryTitle')} className="glossary-modal" onClose={props.onClose} variant="glossary">
      ...
      <div className="routing-action-row"><button className="icon-button routing-primary-action" onClick={props.onSave} type="button"><AppIcon name="save" size={14} />{t('common.save')}</button><button className="icon-button" onClick={props.onClose} type="button"><AppIcon name="close" size={14} />{t('common.cancel')}</button></div>
  </ModalDialog>;
}
