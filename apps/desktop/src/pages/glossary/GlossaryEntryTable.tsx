...
import { formatStrategyTone } from './glossaryPresentation';
...
export default function GlossaryEntryTable(props: Props) {
  ...
  if (!props.selectedLibrary || !props.entries.length) {
    return <div className="glossary-empty">
      ...
      <button className="icon-button routing-primary-action" onClick={props.selectedLibrary ? props.onAddEntry : props.onCreateLibrary} type="button"><AppIcon name={props.selectedLibrary ? 'plus' : 'book'} size={14} />{props.selectedLibrary ? t('glossary.actions.addFirstEntry') : t('glossary.actions.newLibrary')}</button>
    </div>;
  }
  ...
  return <>
    <div className="glossary-table-wrap"><table className="glossary-table">
      ...
      <tbody>{props.entries.map((entry) => <tr key={entry.id}>
        ...
        <td><StatusBadge label={strategyLabel(entry.strategy)} tone={formatStrategyTone(entry.strategy)} /></td>
