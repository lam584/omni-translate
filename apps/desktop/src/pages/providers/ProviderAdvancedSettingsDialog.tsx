  ...
  return <div className="provider-modal-backdrop" onClick={props.onClose} role="presentation">
    <div aria-label={t('providers.actions.advancedSettings')} aria-modal="true" className="provider-modal provider-advanced-modal content-card page-card compact-card" onClick={(event) => event.stopPropagation()} role="dialog">
      ...
      <div className="field-grid provider-field-grid provider-modal-grid">
        ...
        <label className="field-stack"><span>Temperature</span><input className="text-input" max={2} min={0} onChange={(event) => draft({ temperature: Number(event.target.value) || 0 })} step={0.1} type="number" value={props.provider.temperature} /></label>
        <label className="field-stack"><span>Max Output Tokens</span><input className="text-input" min={1} onChange={(event) => draft({ maxOutputTokens: Number(event.target.value) || 1 })} step={1} type="number" value={props.provider.maxOutputTokens} /></label>
