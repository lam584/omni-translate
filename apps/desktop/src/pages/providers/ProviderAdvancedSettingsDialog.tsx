...
export default function ProviderAdvancedSettingsDialog(props: Props) {
  ...
  return <ModalDialog aria-label={t('providers.actions.advancedSettings')} className="provider-modal provider-advanced-modal content-card page-card compact-card" onClose={props.onClose} variant="provider">
      ...
      <section className="provider-setting-block provider-setting-block-compact">
        <div className="provider-setting-header provider-setting-header-compact"><div><strong>{t('providers.customHeaders.title')}</strong></div><button className="icon-button" onClick={props.onHeaderAdd} type="button"><AppIcon name="plus" size={14} />{t('providers.customHeaders.addHeader')}</button></div>
        <div className="provider-custom-header-list">{props.provider.customHeaders.length ? props.provider.customHeaders.map((header) => <div className="provider-custom-header-item" key={header.id}><input className="text-input" onChange={(event) => props.onHeaderChange(header.id, { name: event.target.value })} placeholder={t('providers.customHeaders.namePlaceholder')} value={header.name} /><input className="text-input" onChange={(event) => props.onHeaderChange(header.id, { value: event.target.value })} placeholder={t('providers.customHeaders.valuePlaceholder')} value={header.value} /><select className="select-input" onChange={(event) => props.onHeaderChange(header.id, { enabled: event.target.value === 'true' })} value={String(header.enabled)}><option value="true">{t('providers.common.enabled')}</option><option value="false">{t('providers.common.disabled')}</option></select><button className="provider-header-icon provider-header-icon-danger" onClick={() => props.onHeaderRemove(header.id)} title={t('providers.customHeaders.deleteHeader')} type="button"><AppIcon name="trash" size={13} /></button></div>) : <div className="provider-directory-empty provider-scene-empty"><strong>{t('providers.customHeaders.emptyTitle')}</strong><p>{t('providers.customHeaders.emptyDescription')}</p></div>}</div>
      </section>
  </ModalDialog>;
}
