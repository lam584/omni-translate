  ...
  return <div className="provider-modal-backdrop" onClick={props.onClose} role="presentation">
    <div aria-label={t('providers.pendingModel.title')} aria-modal="true" className="provider-modal provider-advanced-modal content-card page-card compact-card" onClick={(event) => event.stopPropagation()} role="dialog">
      ...
      <div className="field-grid provider-field-grid provider-modal-grid">
        ...
        <label className="field-stack field-span-full"><span>{t('providers.pendingModel.realtimeAudioMode')}</span><select className="select-input" onChange={(event) => props.onChange((current) => current && isRealtimeAudioMode(event.target.value) ? { ...current, realtimeAudioMode: event.target.value } : current)} value={props.pending.realtimeAudioMode}>{realtimeAudioModeOrder.map((mode) => <option key={mode} value={mode}>{formatRealtimeAudioModeLabel(mode)}</option>)}</select></label>
        <div className="field-stack field-span-full"><span>Interaction capabilities</span><div className="provider-scenario-switcher provider-capability-registry-pills">{providerInteractionCapabilityOrder.map((capability) => <button className={props.pending.interactionCapabilities.includes(capability) ? 'provider-scenario-pill provider-scenario-pill-active' : 'provider-scenario-pill'} key={capability} onClick={() => props.onInteractionToggle(capability)} title={formatProviderInteractionCapabilityLabel(capability)} type="button">{formatProviderInteractionCapabilityShortLabel(capability)}</button>)}</div></div>
      </div>
...
export function AudioModeHelpDialog({ onClose }: { onClose: () => void }) {
  ...
  return <div className="provider-modal-backdrop" onClick={onClose} role="presentation">
