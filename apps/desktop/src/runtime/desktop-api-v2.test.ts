...
async function collectEmittedCommands(): Promise<FixtureEntry[]> {
  ...
  const calls: Array<[string, () => Promise<unknown>]> = [
    ...
    ['history.listSessions', () => api.history.listSessions('1710000000000|session-cursor', 25)],
    ['history.getSession', () => api.history.getSession('history-session-1')],
    ['history.listCues', () => api.history.listCues('history-session-1', '50', 50)],
    ['history.getStats', () => api.history.getStats()],
    ['history.deleteSession', () => api.history.deleteSession('history-session-1')],
    ['history.clear', () => api.history.clear()],
    ['history.playCueAudio', () => api.history.playCueAudio('history-session-1', 'cue-1', 'translated')],
    ['history.stopPlayback', () => api.history.stopPlayback()],
...
describe('desktop-api-v2 command fixture (renderer→shell wire pin)', () => {
  ...
  it('covers every v2 envelope action exactly once per service', async () => {
    ...
    expect([...seen].sort()).toEqual([
      ...
      'history_v2:clearHistory',
      'history_v2:deleteSession',
      'history_v2:getSession',
      'history_v2:getStats',
      'history_v2:listCues',
      'history_v2:listSessions',
      'history_v2:playCueAudio',
      'history_v2:stopPlayback',
