...
export type DiagnosticsCommandV2 = { "action": "selfCheck" } | { "action": "overlaySelfCheck" } | { "action": "export", scope: string, } | { "action": "watchSessionReport" } | { "action": "clearWatchSessionReport" } | { "action": "snapshot" } | { "action": "openExportDirectory", outputPath: string, } | { "action": "writeExportArtifact", filename: string, content: string, } | { "action": "openExternalUrl", url: string, };
