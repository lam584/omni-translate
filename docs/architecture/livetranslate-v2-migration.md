...
## September 22: current-runtime local isolation stopped before paid acceptance

Runtime preparation for source `6cb8406120a3b834a00587ffb6ad3b9dd5c4899e` completed and was distributed to all four fixed workers from the clean checkout `E:\ow6`. Preparation `prepare-9ce5f7fc-f814-4309-b70a-d6fd5d91be20` is ready. Moving the checkout outside the main repository resolved standalone Cargo workspace discovery; assigning vm171 a new execution directory preserved the old fixed history/audit binding instead of rewriting it. No Provider calls occurred.

Fresh local isolation `20260922-163027-6cb8406120a3` failed vm171 process-exclusion iteration 0012; the other three formal cells passed, but this does not authorize a paid execution or constitute a successful matrix. The failed assertion was physical Bridge-child fingerprint presence: whole-recording component 0.009551337 versus the unchanged 0.01 minimum. The source exclusion checks did not report leakage.

An independent offline analysis of the retained physical WAV found 319 of 365 nonoverlapping 20 ms Hann windows above the same 0.01 threshold at 2449 Hz, median amplitude 0.359951, while the whole-recording projection was only 0.009567. Absolute-time phase-vector coherence was 0.02417 (compared with 0.19262 in preceding passing iteration 0011). This demonstrates cancellation of physically present tone in the long coherent estimator, not absence of playback. It does not isolate the underlying device discontinuity mechanism or prove a repaired probe passes fresh isolation. Reproduction and numerical results: `artifacts/tmp/local-isolation-phase-20260922/analyze.mjs` and `results.json`. Original failed evidence is retained unchanged. A diagnostic estimator correction requires deterministic phase-gap and negative-control coverage, preserving physical-presence, configured-gain and source-leakage assertions; it is not permission to lower thresholds or relabel this failed run.
The physical estimator correction now uses fifty complete 20 ms local windows and coincident external-tone references for configured translation gain. Existing coherent physical estimates still cap leakage denominators, so leakage ratios cannot decrease. Deterministic tests reject missing, noisy, wrong-frequency, brief and attenuated signals; saved-WAV replay through the actual Rust helper passed for 0011 and 0012. Full Bridge tests/check, bridge integration, contracts, architecture (zero new violations), dead-code audit and the local-isolation/report consumer suites passed. Independent AI review found no actionable issues; required human review remains outstanding. This is offline diagnostic evidence only, not a fresh successful isolation or paid matrix.

## September 22: fresh isolation passed; subsequent Provider preflight was too slow

Current-source local isolation `20260922-181505-16a311cc7d1a` passed all four fixed workers with zero Provider calls, using runtime `qwen3.8-16a311cc-20260922` at source `16a311cc7d1a1af3f932f1e58fbc0b4883cdf5ca`. Its run-specific authority is under `E:\ow6\artifacts\testing\watch-mode-local-isolation`. This fresh result does not relabel the prior failed isolation.

Paid coordinator execution `watch-shard-2fab6a87-31c4-44c1-b80d-c3cde84d0384` then stopped before all audio cells: one Provider session, zero external audio samples, no started cells, cleanup completed without errors. The signed 3.8 zero-audio WebSocket trace completed its six-event lifecycle, but session.created arrived at 3463 ms versus the unchanged 1200 ms budget (upgrade at 3458 ms; session.finished at 4996 ms). The runtime correctly reported latency-budget-exceeded, while the versioned collector incorrectly collapsed this into evidence-outcome-mismatch. Structural wire validation is now separated from strict success eligibility for diagnostic classification only; valid late evidence remains failed with provider.preflight.latency-budget-exceeded, and late tampered evidence remains an authority failure. All existing strict-success consumers retain the 1200 ms limit. Process/authority/selection/coordinator/runtime suites passed 210 tests; contracts and architecture passed.

A separate zero-API DNS/TCP check observed TCP connect times of 548 ms on vm167, 561 ms on vm169 and 68 ms on vm131. vm167 and vm131 resolved the same endpoint address; vm131 also had a Meta default interface while vm167 did not. These are single samples and route-table observations, not proof of effective-route causality or a prescription to change VPN settings. No network configuration changes or automatic paid retries were made. The failed coordinator and its recovered raw evidence remain under `E:\ow6\artifacts\testing\watch-mode-live-coordinator` and `watch-mode-provider-preflight`. Full real 4/4 acceptance and human release review are still outstanding.