...
该命令固定读取 `artifacts/testing/watch-mode-live/latest-successful-watch-mode-strict-matrix.json`。Watch v11 发布验证采用预算平衡方案：复用 3 个零 LLM 本地隔离 authority，再运行 4 个严格 LiveTranslate 付费格。只有这 7 个格子的固定 authority 全部通过，本次 scoped verifier 才会原子替换 canonical manifest；失败、中断、single-device diagnostic 和 `-DryRun` 都不能覆盖它。旧的单进程 strict matrix 入口会在任何 Provider 调用前 fail-closed；正式入口是 `npm run test:watch-mode-live:production-coordinator`。

付费 live 矩阵不再声明统一的 Provider session 秒数。4 个正式格中有 2 个 ordinary 路线（virtual-driver/echo-cancel）和 2 个 process-exclusion 路线（pairwise-live/model-stability）；正常付费媒体墙钟合计为 `2 × 128.8153125 + 2 × 173.8153125 = 605.26125s`。Provider 费用 authority 另按媒体派生的输入样本与 bounded capture grace 计数；`paidLlmSeconds=631.26125` 只是 `10,100,180 / 16,000` 的样本上限等价展示，不是 socket 墙钟、正常等待或会话 hard ceiling：
...
正式模型名以阿里云百炼官方文档 [Qwen3.5-LiveTranslate-Flash-Realtime](https://help.aliyun.com/zh/model-studio/qwen3-5-livetranslate-flash-realtime) 和指定的[百炼控制台文档 2983281](https://bailian.console.aliyun.com/cn-beijing?tab=doc#/doc/?type=model&url=2983281) 为准。Watch v11 正式 preflight 允许北京地域的通用 `wss://dashscope.aliyuncs.com/api-ws/v1/realtime?model=qwen3.5-livetranslate-flash-realtime` 入口，不要额外推导 `WorkspaceId` 必填或 `x-dashscope-workspace` 必需。结束顺序严格遵循同页“结束会话”：输入完成后只发送一次 `session.finish`，继续消费至 `session.finished`，再关闭 WebSocket；跳过该顺序会丢失尾部识别或翻译。

时序上必须区分媒体墙钟、Provider samples、本地 drain 与 watchdog。canonical reference 为 `125.8153125s`，全模式 postroll 为 `3s`；process-exclusion 另在 `90s` 处插入固定 `45s` quiet window，因此 ordinary 媒体墙钟为 `128.8153125s`，process-exclusion 为 `173.8153125s`。`180/225s` 只是 input-complete 失败子期限，不是正常运行时长或成功等待。正常成功由 input-complete marker 及后续 terminal evidence 驱动，Provider finish 上限 15 秒。本地播放 drain 的严格预算为 `min(ceil(pendingPcmFrames * 1000 / outputRateHz) + 2000ms 余量 + 750ms 连续稳定窗, 30000ms)`；缺帧数或输出率时使用配置值并夹在 15–30 秒内 fail-closed。renderer/物理 receipt 连续稳定 750ms 后立即写 immutable report。cell hard watchdog 按模式派生：ordinary 为 `235s`，process-exclusion 为 `280s`；它们只给失败路径封顶。
...
外层超时只包络失败清理，不制造正常等待。runner 的 readiness 和 cell hard 共用 Desktop launch clock，因此使用 `max(90s readiness, cell hard) + 30s report receipt + 20s scheduling`，不将 readiness 与 cell hard 串行相加。Watch v11 的 remote execution hard upper 按 ordinary/process-exclusion 分别为 `505/550s`；每格再加 `60s` lease upload 和 `300s` download/collection envelope 后为 `865/910s`。固定四格的 production coordinator 全局 hard upper 是 `13,102s`，它还覆盖有界 readiness、preflight、清理与最终证据阶段。这些数字全是 fail-closed watchdog/envelope，正常成功仍在 10 个 terminal stages 全部就绪时立即结束。
