...
)
{
    ...
    if (_this->m_ulNotificationIntervalMs > 0 &&
        TimeElapsedInMS >= _this->m_ulNotificationIntervalMs)
    {
    ...
    // Advance the emulated DMA position on every 1 ms hardware tick so the
    // Omni bridge ring receives render PCM continuously. WaveRT completion
    // notifications below still use the negotiated notification interval.
