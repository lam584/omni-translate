...
canonical strict schema-v6 budget-balanced Watch authority and accepts only its fixed
...
seconds of normal media each, and two process-exclusion cells have 173.8153125
seconds each, for 605.26125 seconds of normal paid media in total. The
180/225-second input-completion values are inner failure sub-deadlines, not
normal run durations or successful terminal conditions. Source
...
ceiling and reports zero audio. The supported Beijing control uses the generic
`wss://dashscope.aliyuncs.com/api-ws/v1/realtime?model=qwen3.5-livetranslate-flash-realtime`
endpoint; a `WorkspaceId` or `x-dashscope-workspace` header is not required.
After all
...
complete schema-v6 balanced strict authority verifier; the verification receipt is only
...
| Canonical performance source | `npm run test:watch-mode-live:production-coordinator` | Clean exact HEAD, production binaries, available DashScope credential, one to four schema-v2 workers with pinned BIOS UUID and transport identity, each worker's verified endpoint profile, and a working virtual-driver route; elevation only when an installed driver/device requires it | The signed four-worker plan starts vm171/c01, vm167/c04, vm169/c02, and vm131/c03 at 0/3/6/9-second connection offsets. A one-worker fallback remains serial. Successful cells terminate from eleven raw input/provider/playback/report stages, and each remote execution remains bounded by its signed cell deadline. | Canonical strict manifest, worker-bound zero-Provider local-isolation authorities, 4 passed live `report.json`, and 4 raw `system-metrics.json` files |
...
The canonical file is a schema-v6 authority manifest, not a directory list.
...
matrix through the signed one-to-four-worker production coordinator on Windows. The legacy
