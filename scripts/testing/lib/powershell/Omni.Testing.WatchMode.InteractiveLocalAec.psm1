...
function Resolve-OmniInteractiveLocalAecFields {
  ...
  $names = @('probeRequestPath','probeRequestSha256','outputDirectory','deadlineUtc','desktopExecutable','desktopExecutableSha256','finalizerHelperPath','finalizerHelperSha256','desktopIdentityReporterPath','desktopIdentityReporterSha256','nodeDesktopAuthorityPath')
  ...
  $deadline = [DateTime]::MinValue; if (-not [DateTime]::TryParse([string]$fields.deadlineUtc, [ref]$deadline) -or $deadline.ToUniversalTime() -le [DateTime]::UtcNow) { throw 'interactive local AEC deadline is invalid or expired' }; $outputRoot = [IO.Path]::GetFullPath($fields.outputDirectory).TrimEnd('\') + '\'
