...
const VMIC_PROTOCOL_V6 = '2026-08-10-audio-routing-v6';
...
const validateVirtualMic = (root, options) => {
  ...
  if (probe?.bridgeProtocolVersion !== VMIC_PROTOCOL_V6) issues.push(`virtual microphone Bridge protocol must be ${VMIC_PROTOCOL_V6}`);
  for (const [value, subject] of [
...
const validateInstallFresh = (root, options) => {
  ...
  if (
    state?.protocolVersion !== VMIC_PROTOCOL_V6
...
const validateRepair = (root, options) => {
  ...
  if (
    ...
    || handshake?.protocolVersion !== VMIC_PROTOCOL_V6
